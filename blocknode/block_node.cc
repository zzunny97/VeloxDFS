...
// block_append_local {{{
//bool BlockNode::block_update_local(Block& block, uint32_t pos, uint32_t len, bool replicate) {
bool BlockNode::block_append_local(Block& block, bool replicate) {
  local_io.append(block.first, block.second);

  if (replicate) {
    INFO("Block %s updated real host", block.first.c_str());
    IOoperation io;
    io.operation = messages::IOoperation::OpType::BLOCK_APPEND_REPLICA;
    io.block = move(block);
    replicate_message(&io);
