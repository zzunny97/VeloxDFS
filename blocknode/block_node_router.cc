...
void BlockNodeRouter::io_operation (messages::Message* m_, Channel* tcp_connection) {
  ...
  } else if (m->operation == messages::IOoperation::OpType::BLOCK_APPEND) {
    auto ret = block_node->block_append_local(m->block);
    Reply reply;
  ...
  } else if (m->operation == messages::IOoperation::OpType::BLOCK_APPEND_REPLICA) {
    block_node->block_append_local(m->block, false);
  } 
