...
void Directory::file_table_confirm_upload (std::string file_name, uint32_t num_block, uint32_t num_primary_file, uint64_t size) {
  char sql[DEFAULT_QUERY_SIZE];
//  if(size == 0)
  //	sprintf(sql, "UPDATE file_table set uploading=0, num_block=%u, num_primary_file=%u where name='%s';", num_block, num_primary_file,
   //   file_name.c_str());
 // else
  	sprintf(sql, "UPDATE file_table set uploading=0, num_block=%u, num_primary_file=%u, size=%llu where name='%s';", num_block, num_primary_file, size,
      file_name.c_str());
