...
class DFS {
  public:
	...
	bool write(std::string& file_name, std::string& buf, bool commit = false);
    bool write(std::string& file_name, std::string& buf, uint64_t block_size, bool commit);
	...
	bool write_commit(std::string file_name);
	int tmg_id;
	int shmid;
  	void* shared_memory;
