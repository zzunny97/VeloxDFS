...
namespace velox {
	...
	std::map<std::string, std::shared_ptr<FileInfo>> write_metadata_cache;
	std::map<std::string, std::shared_ptr<string>> write_chunk_cache;
	...
	DFS::DFS(string _mr_job_id, int _tmg_id, bool _initializer) { 
		...
		if(!initializer){
			cout << "Task Manager Id: " << tmg_id << endl;
	...
	bool DFS::write(std::string& file_name, std::string& buf, bool commit) {
		uint64_t BLOCK_SIZE = context.settings.get<int>("filesystem.block");
		return write(file_name, buf, BLOCK_SIZE, commit);
	}

	bool DFS::write(std::string& file_name, std::string& buf, uint64_t block_size, bool commit) {
		//INFO("Start writing %s blocksize %ld", file_name.c_str(), len, off, block_size);
		uint64_t BLOCK_SIZE = context.settings.get<int>("filesystem.block");
		uint32_t file_hash_key = h(file_name);

		if ( write_metadata_cache.find(file_name) == write_metadata_cache.end() ) { 
			if (this->exists(file_name)) {
				cerr << "[ERR] " << file_name << " already exists in VeloxDFS." << endl;
				return EXIT_FAILURE;
			}

			write_metadata_cache.insert({file_name, make_shared<FileInfo>()});
			write_chunk_cache.insert({file_name, make_shared<string>()});
			auto chunk_ptr = write_chunk_cache.find(file_name);
			(chunk_ptr->second)->reserve(block_size);
			*((chunk_ptr)->second) = "";

			FileInfo* file_info = ((write_metadata_cache.find(file_name))->second).get();
			file_info->name = file_name;
			file_info->hash_key = file_hash_key;
			file_info->num_primary_file = 0;
			...
			file_info->blocks_metadata.reserve(NUM_NODES);
			for(int i = 0; i < NUM_NODES; i++) file_info->blocks_metadata.push_back({});

			auto socket = connect(file_hash_key);
			send_message(socket.get(), file_info);

			auto reply = read_reply<FileDescription>(socket.get());
			socket->close();
		}
		
		string& cached_chunk = *((write_chunk_cache.find(file_name))->second);
		
		if((cached_chunk.length() + buf.length() < BLOCK_SIZE) && !commit){
			cached_chunk += buf;
			return true;
		}

		auto fd = write_metadata_cache.find(file_name)->second;
		
		int b_seq;
		unsigned int& n_slab = fd->num_primary_file;
		unsigned int n_chunk = fd->num_block;
		uint64_t buf_len = cached_chunk.length();
		vector<BlockMetadata>& blocks_md = fd->blocks_metadata;
			...
			// Create or Get Slab
			if(n_slab < NUM_NODES){
				cout << "Create Slab : " << n_slab << endl;
				b_seq = n_slab;
				which_server = ((file_hash_key % NUM_NODES) + b_seq) % NUM_NODES;

				blocks_md[which_server].name = file_name + "_primary_" + to_string(which_server);
				blocks_md[which_server].file_name = file_name;
				blocks_md[which_server].hash_key = GET_INDEX_IN_BOUNDARY(which_server);
				blocks_md[which_server].seq = b_seq; 
				blocks_md[which_server].size = 0;
				blocks_md[which_server].replica = replica;
				blocks_md[which_server].node = nodes[which_server];
				blocks_md[which_server].l_node = nodes[(which_server-1+NUM_NODES)%NUM_NODES];
				blocks_md[which_server].r_node = nodes[(which_server+1+NUM_NODES)%NUM_NODES];
				blocks_md[which_server].is_committed = 1;
				...
				b_seq = fd->num_block % NUM_NODES;
				which_server = ((file_hash_key % NUM_NODES) + b_seq) % NUM_NODES;

				chunk_md.offset = blocks_md[which_server].chunks.back().offset + blocks_md[which_server].chunks.back().size;
				io_ops.operation = eclipse::messages::IOoperation::OpType::BLOCK_APPEND;
			}
			
			vector<ChunkMetadata>& chunks_md = blocks_md[which_server].chunks;

			// Create new Chunk
			chunk_md.name = file_name + "_" + to_string(fd->num_block);
			//chunk_md.size = buf.length();
			chunk_md.size = cached_chunk.length();
			chunk_md.foffset = fd->size;
			chunk_md.offset = blocks_md[which_server].size;
			chunk_md.chunk_seq = fd->num_block;
			chunk_md.primary_file = file_name + "_primary_" + to_string(which_server);
			chunk_md.primary_seq = b_seq;
			
			// send buf to blocknode
			io_ops.block.first = chunk_md.primary_file;
			io_ops.block.second = move(cached_chunk);
			auto socket = connect(GET_INDEX(blocks_md[which_server].hash_key));
			send_message(socket.get(), &io_ops);
			auto reply = read_reply<Reply> (socket.get());
			socket->close();

			if(reply->message != "TRUE"){
				cerr << "[ERR] Failed to write block. Details : " << reply->details << endl;
				return false;
			}

		
			chunks_md.push_back(chunk_md);
			blocks_md[which_server].size += buf_len;
		fd->size += chunk_md.size;
		fd->num_block++;
		
		cached_chunk = buf;
		cout << "End of Chunk : " << fd->num_block << " " << chunks_md.size() << endl;

		if(commit) {
			this->write_commit(file_name);
		}
	}
	
	bool DFS::write_commit(std::string file_name){
		auto cached_fd = write_metadata_cache.find(file_name);

		auto fd = cached_fd->second;
		fd->uploading = 0;

		auto socket = connect(h(file_name));
		send_message(socket.get(), fd.get());
		auto reply = read_reply<Reply>(socket.get());

		if(reply->message != "TRUE"){
			cerr << "[ERR] Failed to commit " << file_name << "\nDetails : " << reply->details << endl;
			return false;
		}

		socket->close();
		write_metadata_cache.erase(file_name);
		write_chunk_cache.erase(file_name);
		return true;
	}
