...
void cli_driver::write_test(std::string file){
	std::ifstream ifs(file, std::ios::binary);
	int length = 0, read_length = 0;
...
/*		string str;	
		...
		if(length + line_length >= 8388608){
			dfs->write(file, write_str, false);
			write_str = str;
			length = line_length;
		} else {
			length += line_length;
...
*/
	...
	cout << "Cli Driver : " << read_length << endl;
	if(length != 0)
		dfs->write(file, write_str, false);

	dfs->write_commit(file);
}
