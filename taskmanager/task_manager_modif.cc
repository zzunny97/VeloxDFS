...
TaskManager::TaskManager (ClientHandler* net) : Node() {
	...
	shm_buf_depth = context.settings.get<int>("addons.shm_buf_depth");
	shm_buf_width = context.settings.get<int>("addons.shm_buf_width");
	replica_num = context.settings.get<int>("filesystem.replica");
	buf_pool_size = sizeof(bool) + (sizeof(struct shm_buf) + BLOCK_SIZE) * shm_buf_depth * shm_buf_width;
...
bool TaskManager::alloc_shm(int& shmid, void* &shared_memory){
...
bool TaskManager::create_rw_lock(struct semaphore* &sema, struct shm_buf** &chunk_index, uint64_t& shm_status_addr, 
								 uint64_t& shm_base_addr, uint64_t& shm_chunk_base_addr){
	
	string sema_path = "/tmp/semaphore" + to_string(task_id);
	sema = semaphore_create(sema_path.c_str(), (shm_buf_depth * shm_buf_width) );
	shm_buf** chunk_index = new struct shm_buf*[shm_buf_width * shm_buf_depth]; 
...
bool TaskManager::connect_to_zookeeper_server(zhandle_t* &zh, string& job_id, string& zk_prefix, string& zk_eof ){
...
void task_worker(std::string file, struct logical_block_metadata& lblock_metadata, string _job_id, int _task_id){
	...
	int shmid;
	void* shared_memory; 
	alloc_shm(shmid, shared_memory);

	/* For User level semaphore */
	struct shm_buf** chunk_index;
	struct semaphore *sema;
	uint64_t shm_status_addr, shm_base_addr, shm_chunk_base_addr;
	create_rw_lock(sema, chunk_index, shm_status_addr, shm_base_addr, shm_chunk_base_addr){
	
	/* For Zookeepr */	
	zhandle_t* zh;
	string zk_prefix, zk_eof;
	connect_to_zookeeper_server(zh, job_id, zk_prefix, zk_eof);
	...
	int processed_file_cnt = 0, r_idx = 0, shm_idx = 0;
	int process_chunk = 0;
